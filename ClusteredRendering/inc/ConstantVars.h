...
namespace Constants
{
	...
	const float SPECIAL_NEAR = 5.0f;

	const uint16_t NUM_X_CLUSTERS = 16;
	const uint16_t NUM_Y_CLUSTERS = 9;
	const uint16_t NUM_Z_CLUSTERS = 16;

	const uint16_t NUM_X_PLANES = (NUM_X_CLUSTERS + 1);
	const uint16_t NUM_Y_PLANES = (NUM_Y_CLUSTERS + 1);
	const uint16_t NUM_Z_PLANES = (NUM_Z_CLUSTERS + 1);
